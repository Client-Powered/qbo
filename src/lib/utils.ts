...
export const getJson = <T extends { intuitTid: number | null }>() => async (res: Response): Promise<Result<T, QBOError>> => {
  const maybeIntuitTid = res?.headers?.get("intuit_tid") ;
  const intuitTid = maybeIntuitTid
    ? Number(maybeIntuitTid)
    : null;
  if (!res.ok) {
